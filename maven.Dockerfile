...
COPY .mvn ./.mvn
COPY pom.xml mvnw .
RUN  ls && ./mvnw clean install -Dmaven.test.skip=true -Dmaven.wagon.http.ssl.insecure=true -Dmaven.wagon.http.ssl.allowall=true -Dmaven.wagon.http.ssl.ignore.validity.dates=true -Dcheckstyle.skip=true
COPY src .
